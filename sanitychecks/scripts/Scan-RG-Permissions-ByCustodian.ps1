...
function CustFromSub([string]$n){
  if($n -match '(?i)ADH([A-Za-z0-9_-]+)'){ $v=$Matches[1] -replace '^[^A-Za-z0-9]+',''; if($v -match '^([A-Za-z0-9]+)'){return $Matches[1]} return $v }
  $null
}
...
if(-not $subs){ throw "No subscriptions with 'ADH' and '$adh_group' in name." }
...
foreach($sub in $subs){
  ...
  foreach($e in $expected){
    ...
    if(-not $rgObj){
      $rows.Add([pscustomobject]@{SubscriptionName=$sub.Name;SubscriptionId=$sub.Id;Environment=$env
        InputResourceGroup=$inRG;ScannedResourceGroup=$rg;RoleDefinition=$rl;InputAdGroup=$inGrp;ResolvedAdGroup=$gp;GroupObjectId=''
        RGStatus='NOT_FOUND';PermissionStatus='N/A_RG_NOT_FOUND';Status='RG_NOT_FOUND';Details='RG not found'}); continue
    }
    ...
    if(-not $g){
      $rows.Add([pscustomobject]@{SubscriptionName=$sub.Name;SubscriptionId=$sub.Id;Environment=$env
        InputResourceGroup=$inRG;ScannedResourceGroup=$rg;RoleDefinition=$rl;InputAdGroup=$inGrp;ResolvedAdGroup=$gp;GroupObjectId=''
        RGStatus='EXISTS';PermissionStatus='N/A_GROUP_NOT_FOUND';Status='GROUP_NOT_FOUND';Details='Group not found'}); continue
    }
    ...
    if($ra){
      $rows.Add([pscustomobject]@{SubscriptionName=$sub.Name;SubscriptionId=$sub.Id;Environment=$env
        InputResourceGroup=$inRG;ScannedResourceGroup=$rg;RoleDefinition=$rl;InputAdGroup=$inGrp;ResolvedAdGroup=$gp;GroupObjectId=$g.Id
        RGStatus='EXISTS';PermissionStatus='EXISTS';Status='EXISTS';Details='' })
    } else {
      $rows.Add([pscustomobject]@{SubscriptionName=$sub.Name;SubscriptionId=$sub.Id;Environment=$env
        InputResourceGroup=$inRG;ScannedResourceGroup=$rg;RoleDefinition=$rl;InputAdGroup=$inGrp;ResolvedAdGroup=$gp;GroupObjectId=$g.Id
        RGStatus='EXISTS';PermissionStatus='MISSING';Status='MISSING';Details='Role assignment missing at RG' })
    }
  }
}
...
if($TeamsWebhookUrl){
  ...
  $sum="RG Permissions ($adh_group) $stamp`nTotal:$($rows.Count) Exists:$ok Missing:$mi RG_NA:$rgna Group_NA:$ggna`n$permCsv`n$permHtml"
