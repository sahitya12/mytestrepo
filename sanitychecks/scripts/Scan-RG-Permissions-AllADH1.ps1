<# 
.SYNOPSIS
  Scan RG-level RBAC for all subscriptions whose names contain "ADH".
  Compares against expected permissions from two CSVs (PROD vs NONPROD).
  <Custodian> tokens are replaced using the custodian parsed from each subscription name (ADH<XYZ>).
  If not present in the name, falls back to -adh_group (optional).

.PARAMETER TenantId, ClientId, ClientSecret
  Service principal credentials.

.PARAMETER ProdCsvPath, NonProdCsvPath
  CSVs with columns: resource_group_name,role_definition_name,ad_group_name.

.PARAMETER adh_group
  Optional fallback for <Custodian> replacement.

.PARAMETER OutputDir, TeamsWebhookUrl
  Outputs (CSV/HTML) and optional Teams webhook.
#>
...
# ---- Subs: all with ADH in name ----
$subs = Get-AzSubscription | ? { $_.Name -match '(?i)ADH' }
if (-not $subs) { throw "No subscriptions with 'ADH' in the name were found." }
...
foreach ($sub in $subs) {
  ...
  if (-not $subCust -and $adh_group) { $subCust = $adh_group }
...
if ($TeamsWebhookUrl) {
  ...
  $sum  = "RG Permissions (ALL ADH) $stamp`nTotal:$($rows.Count) Exists:$ok Missing:$miss RG_NA:$rgna Group_NA:$gpna`n$permCsv`n$permHtml"
