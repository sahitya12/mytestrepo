<# 
.SYNOPSIS
  Inventory RG tags. Supports:
   -ScanAll (all subs with "ADH" in the name), or
   -CustodianFilter <code> (only subs whose name contains ADH + this code), or
   -SubscriptionIds <id1,id2,...> (explicit).

  Exports CSV/HTML. Can check for required tag keys (owner, environment, costcenter by default).

.PARAMETER TenantId, ClientId, ClientSecret
  Service principal credentials.

.PARAMETER ScanAll
  If set, scans all subscriptions that contain "ADH" in the name.

.PARAMETER CustodianFilter
  If provided, only subscriptions whose name contains both "ADH" and this value are scanned.

.PARAMETER SubscriptionIds
  Explicit list of subscription IDs to scan (overrides other filters).

.PARAMETER OutputDir, TeamsWebhookUrl
  Output folder and optional Teams webhook.

.PARAMETER RequiredTagKeys
  A list of tag keys to verify exist on each RG (case-insensitive).

.PARAMETER PerTagRows
  If set, writes one row per tag (TagName/TagValue) instead of one row per RG.
#>
...
function OutDir([string]$d) {
  if ([string]::IsNullOrWhiteSpace($d)) { $d = Join-Path (Get-Location) 'rg-tags-out' }
  if (-not (Test-Path $d)) { New-Item -ItemType Directory -Path $d | Out-Null }
  $d
}
function TagFlat($t) { if (-not $t) { return '' }; ($t.GetEnumerator() | % { "$($_.Key)=$($_.Value)" }) -join '; ' }
function HasTag($t,[string]$k) {
  if (-not $t) { return $false }
  foreach ($x in $t.Keys) { if ($x -ieq $k) { return $true } }
  $false
}
  ...
  foreach ($rg in $rgs) {
    if ($PerTagRows) {
      ...
      $base = [ordered]@{
        ...
        TagsFlat         = (TagFlat $rg.Tags)
      }
      foreach ($k in $RequiredTagKeys) {
        $base["Has_$($k)"] = (HasTag $rg.Tags $k)
      }
...
Write-Host "RG Tags CSV : $csv"
Write-Host "RG Tags HTML: $html"

# ---- Teams (optional) ----
if ($TeamsWebhookUrl) {
  $rgCount  = ($rows | Select-Object -Expand ResourceGroup -Unique | Measure-Object).Count
  $subCount = ($rows | Select-Object -Expand SubscriptionId -Unique | Measure-Object).Count
  $summary  = "RG Tag Scan $stamp`nSubscriptions:$subCount`nRGs(unique):$rgCount`n$csv`n$html"
  try { Invoke-RestMethod -Method Post -Uri $TeamsWebhookUrl -ContentType 'application/json' -Body (@{ text = $summary } | ConvertTo-Json) }
